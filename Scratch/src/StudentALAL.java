 ...
 *
 * This class is used to create an ArrayList<ArrayList<midterm.Student>>, which is used to subdivide the group of all midterm.Student objects
...
public class StudentALAL
{
     ...
     * Used to initialize the subgroups ArrayList<ArrayList<midterm.Student>>
     */
    StudentALAL ()
    {
     ...
     * number of Students, an Exception is thrown. Otherwise, the method adds the appropriate number of ArrayList<midterm.Student>
     * to the subgroups ArrayList<ArrayList<midterm.Student>>.  Next, the Students are shuffled and randomly added to the newly created
     * subgroups, using the Random class and a 'for' loop. As Students are added to the subgroups, they are removed
     * from the original ArrayList<midterm.Student> which contained all Students.
     * @param studentAL
     * @throws IOException
     */
    public void createGroups (StudentAL studentAL) throws IOException
    {
        ...
        //For loop that creates appropriate number of subgroups (ArrayList<midterm.Student>)
        ...
        while (studentAL.getTotalStudents() > 0)
        {
            ...
            for (int i = 0; i < numGroups; i++)
            {
                ...
                if (studentAL.getTotalStudents() > 0)
                {
                    ...
                    //Add a midterm.Student at the (random) index to the next subgroup in line, while
                    //removing midterm.Student from the ArrayList<midterm.Student> of all Students
    ...
    public String toString ()
    {
        ...
        for (int i = 0; i < subgroups.size(); i++)
        {
            ...
            //Iterate through each midterm.Student object in the subgroup, and add the midterm.Student to the String
